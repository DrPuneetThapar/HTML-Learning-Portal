...
function renderPicker(){
  ...
  QZ_TOPICS.forEach(t=>{
    ...
    btn.innerHTML = `<b>${t.title}</b><span class="qcount">${qs.length} questions${score ? ' · last score ' + score.score + '/' + score.total : ''}</span>`;
...
function renderQuestion(){
  ...
  card.innerHTML = `
    ...
    <div class="quiz-explain-box" id="quizExplainBox">${q.explain}</div>
  ...
  q.opts.forEach((optText, oi)=>{
    ...
    b.innerHTML = `<span class="quiz-option-letter">${letters[oi]}</span>${optText}`;
...
function showResults(){
  ...
  s.answers.forEach(a=>{
    ...
    div.innerHTML = `<span class="status">${a.correct ? "✓" : "✗"}</span>${a.q} ${a.correct ? "" : `<br><span class="text-muted">You chose "${a.chosen}" — correct answer: "${a.correctText}"</span>`}`;
